...
import type { Conversation, ConversationMetadata, Message } from "@/types";
      ...
      <div className="flex flex-col h-screen w-full bg-black text-white p-4">
        ...
        <div className="flex flex-col-reverse gap-4 h-full p-4 items-center overflow-y-auto scroller">
          {conversation?.messages?.toReversed().map((chatMessage) => (
            ...
            >
              <div
                className={`flex items-start gap-2 ${chatMessage.isUser ? "flex-row-reverse" : ""}`}
              >
                {!chatMessage.isUser && (
                ...
                <div
                  className={`flex flex-col ${chatMessage.isUser ? "items-end" : ""}`}
                >
                  <h3 className="font-bold text-neutral-400">
                    {chatMessage.isUser ? "You" : "Spacey"}
                  </h3>
                  <div
                    className={`${chatMessage.isUser ? "bg-blue-600 rounded-xl px-4 py-2" : ""}`}
                  >
        ...
        <div className="flex flex-col w-full h-fit gap-2 border border-neutral-800 rounded-lg p-4 bg-neutral-950">
          ...
          <div className="flex items-center gap-2 text-sm text-neutral-400">
            <label
              htmlFor="file-upload"
              className="flex items-center gap-1 cursor-pointer hover:text-neutral-300"
            >
